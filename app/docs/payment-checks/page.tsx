...
export default function PaymentChecksPage() {
 ...
 <h3 className="font-semibold text-foreground mb-1">Fully Gasless</h3>
 ...
 <h3 className="font-semibold text-foreground mb-1">Single Key Transfer</h3>
 <p className="text-sm text-muted-foreground">
 The entire check is represented by a single <code className="text-xs bg-card-muted px-1 rounded">check_key</code> &mdash;
 ...
 <div className="border border-border rounded-lg p-5">
 <div className="text-2xl mb-2">&#128200;</div>
 <h3 className="font-semibold text-foreground mb-1">Partial Claims &amp; Reclaim</h3>
 <p className="text-sm text-muted-foreground">
 ...
 <div className="text-xs font-bold text-foreground">Agent A (Sender)</div>
 <div className="text-xs text-muted-foreground mt-1">Has wallet API key</div>
              </div>
 <div className="flex flex-col items-center justify-center pt-4">
 <div className="text-xs text-muted-foreground font-medium">check_key</div>
 <div className="flex items-center gap-1">
 <div className="h-px w-16 bg-card-muted"></div>
 <svg width="12" height="12" className="text-faint-foreground"><path d="M0 6 L8 6 M4 2 L8 6 L4 10" stroke="currentColor" strokeWidth="2" fill="none"/></svg>
                </div>
 <div className="text-[10px] text-faint-foreground">any channel</div>
              </div>
 <div className="bg-card rounded-lg border-2 border-border p-3 text-center">
 <div className="text-lg mb-1">&#129302;</div>
 <div className="text-xs font-bold text-foreground">Agent B (Receiver)</div>
 <div className="text-xs text-muted-foreground mt-1">Has wallet API key</div>
              </div>
            </div>
 ...
 <span className="text-[10px] text-accent-text font-medium">create check</span>
              </div>
              <div />
 <div className="flex flex-col items-center">
 ...
 <span className="text-[10px] text-foreground font-medium">claim check</span>
              </div>
            </div>

            {/* Row 3: TEE + Ephemeral */}
 <div className="relative border-2 border-border rounded-lg p-4 bg-card-muted/50">
 <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-card-muted0 text-white text-xs font-bold px-3 py-0.5 rounded-full whitespace-nowrap">TEE (Intel TDX)</span>
 <div className="grid grid-cols-3 gap-4 mt-2">
 <div className="bg-card rounded-lg border border-border p-3 text-center">
 <div className="text-xs font-semibold text-foreground">Ephemeral Key</div>
 <div className="text-xs text-muted-foreground mt-1">Derived per check</div>
 <div className="text-xs text-muted-foreground">HMAC from master secret</div>
                </div>
 <div className="bg-card rounded-lg border border-border p-3 text-center">
 <div className="text-xs font-semibold text-foreground">NEP-413 Signing</div>
 <div className="text-xs text-muted-foreground mt-1">Off-chain intent signatures</div>
 <div className="text-xs text-muted-foreground">No gas required</div>
                </div>
 <div className="bg-card rounded-lg border border-border p-3 text-center">
 <div className="text-xs font-semibold text-foreground">Reclaim</div>
 <div className="text-xs text-muted-foreground mt-1">Re-derives ephemeral key</div>
 <div className="text-xs text-muted-foreground">Sender never stores key</div>
                </div>
              </div>
            </div>
 ...
 <div className="flex flex-col items-center">
 <svg width="24" height="28" className="text-muted-foreground"><path d="M12 0 L12 20 M6 14 L12 20 L18 14" stroke="currentColor" strokeWidth="2" fill="none"/></svg>
 <span className="text-[10px] text-muted-foreground font-medium">publish_intent (gasless)</span>
              </div>
            </div>
 ...
 <div className="bg-card rounded-lg border-2 border-border p-3 text-center">
 <div className="text-xs font-bold text-muted-foreground">Solver Relay</div>
 <div className="text-xs text-muted-foreground mt-1">Executes signed transfer intents</div>
 <div className="text-xs text-muted-foreground">No gas from sender or receiver</div>
              </div>
 <div className="bg-card rounded-lg border-2 border-border p-3 text-center">
 <div className="text-xs font-bold text-muted-foreground">intents.near</div>
 <div className="text-xs text-muted-foreground mt-1">Token balances (multi-token)</div>
 <div className="text-xs text-muted-foreground">Ephemeral accounts as escrow</div>
              </div>
            </div>
 ...
 <ol className="list-decimal list-inside text-foreground space-y-3 mb-4">
          <li>
 <strong>Create</strong> &mdash; Agent A calls <code className="text-xs bg-card-muted px-1 rounded">POST /payment-check/create</code>.
 ...
 and returns a <code className="text-xs bg-card-muted px-1 rounded">check_key</code>.
          </li>
          <li>
 <strong>Share</strong> &mdash; Agent A sends the <code className="text-xs bg-card-muted px-1 rounded">check_key</code> to Agent B
            over any channel (API call, message, QR code). The key is a 64-char hex string &mdash; compact and easy to transmit.
          </li>
          <li>
 <strong>Claim</strong> &mdash; Agent B calls <code className="text-xs bg-card-muted px-1 rounded">POST /payment-check/claim</code> with the key.
 ...
 and submits them to the solver relay, which executes the transfer on <code className="text-xs bg-card-muted px-1 rounded">intents.near</code>.
        </p>
 ...
 <td className="px-4 py-2 text-foreground font-medium">None</td>
              </tr>
 <tr className="border-b">
 ...
 <strong>Why ephemeral accounts?</strong> Each check gets its own ephemeral account on <code className="text-xs bg-card-muted px-1 rounded">intents.near</code> &mdash;
...
The ephemeral account ID = hex(public_key) on intents.near.`}
 ...
 <h3 className="font-semibold text-foreground mb-2">Agent-to-Agent Payments</h3>
 <p className="text-sm text-muted-foreground">
 ...
 <div className="border border-border rounded-lg p-4">
 <h3 className="font-semibold text-foreground mb-2">Bounties &amp; Task Rewards</h3>
 <p className="text-sm text-muted-foreground">
 ...
 <div className="border border-border rounded-lg p-4">
 <h3 className="font-semibold text-foreground mb-2">Escrow-Style Payments</h3>
 <p className="text-sm text-muted-foreground">
 ...
 <div className="border border-border rounded-lg p-4">
 <h3 className="font-semibold text-foreground mb-2">Batch Payouts</h3>
 <p className="text-sm text-muted-foreground">
 Use <code className="text-xs bg-card-muted px-1 rounded">batch-create</code> to generate up to 10 checks in a single call.
 ...
 <p className="text-foreground mb-4">All examples use the OutLayer API at <code className="text-xs bg-card-muted px-1 rounded">https://api.outlayer.ai</code>.</p>

 <h3 className="font-semibold text-foreground mb-3">1. Create a check</h3>
...
{`{
 ...
 Send <code className="text-xs bg-card-muted px-1 rounded">check_key</code> to the other agent via any channel.
          The key is all they need to claim the funds.
        </p>

 <h3 className="font-semibold text-foreground mb-3">3. Receiver claims the check</h3>
 <SyntaxHighlighter language="bash" style={vscDarkPlus} className="rounded-lg text-sm mb-4">
...
{`{
 ...
 <p className="text-foreground mb-4">
 Base URL: <code className="text-xs bg-card-muted px-1 rounded">https://api.outlayer.ai/wallet/v1/payment-check</code>.
 All endpoints require wallet API key authentication via <code className="text-xs bg-card-muted px-1 rounded">Authorization: Bearer wk_...</code> header.
        </p>

        {/* POST /create */}
 <div className="border border-border rounded-lg mb-4">
 <div className="bg-card-muted px-4 py-2 border-b border-border flex items-center gap-2">
 <span className="text-xs font-bold text-white bg-success px-2 py-0.5 rounded">POST</span>
 <code className="text-sm font-mono">/create</code>
          </div>
 <div className="p-4">
 <p className="text-sm text-foreground mb-3">Create a new payment check. Locks tokens from the sender&apos;s intents balance into an ephemeral account.</p>
 ...
 <div className="p-4">
 <p className="text-sm text-foreground mb-3">Create multiple checks in a single call (max 10). Each check gets independent key and ephemeral account.</p>
 <p className="text-xs font-semibold text-muted-foreground mb-1">Request body</p>
 <div className="overflow-x-auto mb-3">
 <table className="min-w-full text-xs border border-border">
 <thead className="bg-card-muted">
 ...
 <p className="text-xs font-semibold text-muted-foreground mb-1">Response</p>
 <p className="text-xs text-muted-foreground">Returns <code className="bg-card-muted px-1 rounded">{`{ checks: [...] }`}</code> &mdash; array of create responses.</p>
          </div>
        </div>
 ...
 <div className="p-4">
 <p className="text-sm text-foreground mb-3">Claim funds from a check. Transfers from ephemeral account to the caller&apos;s wallet. Supports partial claims.</p>
 <p className="text-xs font-semibold text-muted-foreground mb-1">Request body</p>
 <div className="overflow-x-auto mb-3">
 <table className="min-w-full text-xs border border-border">
 <thead className="bg-card-muted">
 ...
 <div className="p-4">
 <p className="text-sm text-foreground mb-3">Reclaim unclaimed funds back to the sender&apos;s wallet. Only the original creator can reclaim. Supports partial reclaim.</p>
 <p className="text-xs font-semibold text-muted-foreground mb-1">Request body</p>
 <div className="overflow-x-auto mb-3">
 <table className="min-w-full text-xs border border-border">
 <thead className="bg-card-muted">
 ...
 <div className="border border-border rounded-lg mb-4">
 <div className="bg-card-muted px-4 py-2 border-b border-border flex items-center gap-2">
 <span className="text-xs font-bold text-white bg-info px-2 py-0.5 rounded">GET</span>
 <code className="text-sm font-mono">/status?check_id=...</code>
          </div>
 <div className="p-4">
 <p className="text-sm text-foreground mb-3">Get the current status of a check. Only the creator can query by check_id.</p>
 ...
 <p className="text-xs font-semibold text-muted-foreground mb-1">Response</p>
 <p className="text-xs text-muted-foreground">Returns full check details: <code className="bg-card-muted px-1 rounded">check_id</code>, <code className="bg-card-muted px-1 rounded">token</code>, <code className="bg-card-muted px-1 rounded">amount</code>, <code className="bg-card-muted px-1 rounded">claimed_amount</code>, <code className="bg-card-muted px-1 rounded">reclaimed_amount</code>, <code className="bg-card-muted px-1 rounded">status</code>, <code className="bg-card-muted px-1 rounded">memo</code>, <code className="bg-card-muted px-1 rounded">created_at</code>, <code className="bg-card-muted px-1 rounded">expires_at</code>, <code className="bg-card-muted px-1 rounded">claimed_at</code>, <code className="bg-card-muted px-1 rounded">claimed_by</code>.</p>
          </div>
        </div>
 ...
 <div className="p-4">
 <p className="text-sm text-foreground mb-3">List all checks created by the authenticated wallet. Supports filtering and pagination.</p>
 <p className="text-xs font-semibold text-muted-foreground mb-1">Query parameters</p>
 <div className="overflow-x-auto mb-3">
 <table className="min-w-full text-xs border border-border">
 <thead className="bg-card-muted">
 ...
 <p className="text-xs font-semibold text-muted-foreground mb-1">Response</p>
 <p className="text-xs text-muted-foreground">Returns <code className="bg-card-muted px-1 rounded">{`{ checks: [...] }`}</code> &mdash; array of status responses.</p>
          </div>
        </div>
 ...
 <div className="p-4">
 <p className="text-sm text-foreground mb-3">Check a payment check&apos;s balance and status using the check_key. Use this to verify a check before claiming.</p>
 <p className="text-xs font-semibold text-muted-foreground mb-1">Request body</p>
 <div className="overflow-x-auto mb-3">
 <table className="min-w-full text-xs border border-border">
 <thead className="bg-card-muted">
 ...
 <p className="text-xs font-semibold text-muted-foreground mb-1">Response</p>
 <p className="text-xs text-muted-foreground">Returns <code className="bg-card-muted px-1 rounded">token</code>, <code className="bg-card-muted px-1 rounded">balance</code> (on-chain), <code className="bg-card-muted px-1 rounded">memo</code>, <code className="bg-card-muted px-1 rounded">status</code>, <code className="bg-card-muted px-1 rounded">expires_at</code>.</p>
          </div>
        </div>
      </section>
...
Statuses:
 ...
 <div className="bg-card-muted border-l-4 border-yellow-500 p-4 mb-4">
 <p className="text-sm text-foreground">
 <strong>Expiry:</strong> If <code className="text-xs bg-card-muted px-1 rounded">expires_in</code> is set, the check cannot be claimed after expiry.
 ...
 <div className="bg-card-muted border-l-4 border-border p-4">
 <p className="text-sm text-foreground">
 ...
 <td className="px-4 py-2 text-foreground">None</td>
 <td className="px-4 py-2 text-destructive-text">Sender pays</td>
 <td className="px-4 py-2 text-destructive-text">Both parties pay</td>
              </tr>
 <tr className="border-b">
 <td className="px-4 py-2 font-medium">Receiver needs account</td>
 <td className="px-4 py-2 text-foreground">Only a wallet API key</td>
 <td className="px-4 py-2 text-destructive-text">On-chain account + gas</td>
 <td className="px-4 py-2 text-destructive-text">On-chain account + gas</td>
              </tr>
 <tr className="border-b">
 <td className="px-4 py-2 font-medium">Partial payment</td>
 <td className="px-4 py-2 text-foreground">Built-in</td>
 <td className="px-4 py-2 text-destructive-text">N/A</td>
 <td className="px-4 py-2 text-foreground">Custom logic</td>
              </tr>
 <tr className="border-b">
 <td className="px-4 py-2 font-medium">Reclaim</td>
 <td className="px-4 py-2 text-foreground">Built-in, gasless</td>
 <td className="px-4 py-2 text-destructive-text">N/A (irreversible)</td>
 <td className="px-4 py-2 text-foreground">Custom logic + gas</td>
              </tr>
 <tr className="border-b">
 <td className="px-4 py-2 font-medium">On-chain footprint</td>
 <td className="px-4 py-2 text-foreground">Solver relay only</td>
 <td className="px-4 py-2 text-foreground">1 transaction</td>
 <td className="px-4 py-2 text-destructive-text">Contract deployment + calls</td>
              </tr>
 <tr className="border-b">
 <td className="px-4 py-2 font-medium">Setup complexity</td>
 <td className="px-4 py-2 text-foreground">One API call</td>
 <td className="px-4 py-2 text-foreground">One API call</td>
 <td className="px-4 py-2 text-destructive-text">Contract dev + deployment</td>
              </tr>
            </tbody>
          </table>
        </div>
