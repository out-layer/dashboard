...
export default function VaultsDocsPage() {
 ...
 <h1 className="text-4xl font-bold mb-3">MPC Vaults</h1>
      ...
      {/* ── 0. Quick CKD primer ──────────────────────────────────────── */}
 <div className="bg-card-muted border-l-4 border-border p-4 mb-6 text-sm">
 ...
 ephemeral on OutLayer is <strong>not stored anywhere</strong>. It is
          ...
          disk &mdash; on every keystore start it is requested from <strong>NEAR&rsquo;s MPC network</strong> via a primitive called <strong>CKD</strong> (Conditional Key Derivation): threshold-key
 ...
 See <Link href="/docs/agent-custody" className="text-foreground underline">Agent Custody</Link> and <Link href="/docs/secrets" className="text-foreground underline">Secrets</Link> for
 ...
 <p className="text-foreground mb-3">
        ...
        key scoped to a single proxy method that calls NEAR MPC&rsquo;s <code>request_app_private_key</code>. As long as that key is in
 ...
 <th className="text-left px-3 py-2 font-semibold bg-card-muted">MPC vault (yours)</th>
            </tr>
          </thead>
 ...
 <td className="px-3 py-2 align-top bg-card-muted/50">
                Your vault contract
              </td>
            </tr>
 <tr className="border-t border-border">
 ...
 <td className="px-3 py-2 align-top bg-card-muted/50">1 NEAR at deploy (storage + working balance for the on-chain MPC derive; keep ≥0.5 NEAR)</td>
            </tr>
          </tbody>
        </table>
      </div>

 <div className="border-l-4 border-border bg-card-muted p-4 mb-6">
 <h4 className="font-semibold mb-2 text-foreground">Two modes &mdash; one-way switch</h4>
 ...
 call <code>finalize_recovery(new_parent_pubkey)</code>              with a key you generated locally. The contract atomically
 ...
 <p className="text-sm text-foreground mb-0">
          ...
          itself. The TEE keystore calls <code>request_app_private_key</code> through the vault&rsquo;s
...
outlayer vault init --name treasury --exit-window 7d`}
 ...
 <li>One transaction, five actions: <code>CreateAccount</code>{' + '}<code>Transfer 1 NEAR</code>{' + '}<code>UseGlobalContract(code_hash)</code>{' + '}<code>new(parent, keystore_dao, mpc_contract, initial_tee_pubkey, initial_exit_window)</code>{' + '}<code>AddKey(tee_pubkey, FCAK on vault.request_master)</code>.</li>
          <li>
 CLI/dashboard calls <code>POST /customer/register</code>            on the coordinator. The coordinator forwards to the
            keystore-worker, which independently re-runs the five RPC
            verification checks and signs <code>mark_vault_verified</code> on the DAO so the vault
 ...
 <p className="text-sm text-muted-foreground">
          ...
          coordinator&rsquo;s sign-verification call), use <code>outlayer vault resume &lt;account&gt;</code> or the <strong>Resume</strong> button on the dashboard.
 ...
 <p className="mb-3 text-sm text-foreground">
          To get a wallet API key bound to your vault, run <code>POST /register</code> with <code>{`{"vault_id": "<vault>"}`}</code> separately:
        </p>
        <SyntaxHighlighter language="bash" style={vscDarkPlus}>
...
#           near_account_id: "<hex>", handoff_url, trial }`}
 ...
 coordinator stores <code>wallet_accounts(wallet_id, vault_id)</code>          and N wallets per vault are allowed. The wallet&rsquo;s NEAR
          implicit address comes from <code>HMAC-SHA256(per_vault_master, &quot;wallet:&#123;wallet_id&#125;:near&quot;)</code>,
 ...
 <p className="mb-3 text-sm text-foreground">
          Every wallet API call sends <code>Authorization: Bearer wk_...</code>. The coordinator
          looks up the API key in its DB, finds the bound <code>wallet_id</code> + <code>vault_id</code>, and forwards <code>X-Customer-Vault: &lt;vault_id&gt;</code> to the
          keystore on every signing call. The vault scope is <strong>auth-driven, never request-driven</strong>: a
 client-supplied <code>X-Customer-Vault</code> header on a
          /wallet/v1 call is ignored — the binding lives on the API
          key&rsquo;s DB row. (Test: <code>tests/vault_multi_customer_isolation.sh</code>.)
        </p>
 ...
 <p className="mb-3 text-sm text-foreground">
 <strong>The deterministic <code>/register</code> path</strong> (5-tuple: <code>account_id + seed + pubkey + message + signature</code>,
          where the user proves possession of a NEAR key) does <strong>not</strong> support <code>vault_id</code>. Passing
 ...
 anyone can call <code>initiate_recovery</code> on the vault.
          ...
          starts a fixed seven-day timer, then a seven-day window for <code>finalize_recovery</code>. <strong>Only the parent
...
# Vault is now unlocked; your key is the only FullAccess key.`}
 ...
 time. Like cessation, <strong>finalize is parent-only</strong>          and atomically swaps OutLayer&rsquo;s TEE keys for <code>new_parent_pubkey</code>.
        </p>
        <SyntaxHighlighter language="bash" style={vscDarkPlus}>
...
outlayer vault finalize-recovery             vault.alice.near "$NEW_PUBKEY"`}
 ...
 <p className="text-sm text-foreground mt-3">
          Optional follow-up: <code>outlayer vault unlocked-add-key vault.alice.near ed25519:...</code> adds <em>additional</em> keys (e.g. function-call keys for
          day-to-day ops) on top of the new parent key that <code>finalize_recovery</code> already installed.
        </p>

 <div className="bg-card-muted border border-border rounded p-3 mt-4 text-sm text-foreground">
 ...
 <p className="text-sm text-foreground mb-3">
          ...
          the unlocked vault can submit a fresh <code>request_app_private_key</code> to the MPC contract and
          arrive at the same 32-byte master. The standalone          <a
 ...
 <code>customer-recovery</code>
          </a>          binary does that, plus two helpers:
        </p>
 ...
 the coordinator returned at <code>/register</code> time;
          ...
          <li>
            ...
            (pre-v0.2 CLI). The ciphertext comes from <code>get_secrets(accessor, profile, owner)</code> on the
 ...
 <code>docs/LEAVING_OUTLAYER.md</code>
          ...
          . The wrapper script <code>scripts/customer-recovery/walkthrough.sh</code> runs
...
# stdout: master_hex=<64 hex>`}
 ...
 <strong>Coordinator-side fast-fail.</strong> Once the vault
          is unlocked on chain, the OutLayer coordinator refuses any <code>/call/&lt;owner&gt;/&lt;project&gt;</code> request that
          touches a secret bound to it with <strong>HTTP 423 Locked</strong> (body <code>{`{reason: "vault_unlocked", vault_id, error}`}</code>)
 ...
 the vault provides. After waiting <code>unilateral_exit_window_secs</code>
          ...
          on chain), the customer&rsquo;s parent account calls <code>finalize_recovery(new_parent_pubkey)</code>: the
 ...
 <div className="bg-card-muted border border-border rounded p-3 text-sm text-foreground">
 <strong>Practical guidance:</strong>
 <ul className="list-disc list-inside mt-1 space-y-1">
 <li>Read <code>unilateral_exit_window_secs</code> with <code>outlayer vault verify</code> BEFORE depositing.
 ...
 <li>The vault&rsquo;s <code>recovery</code> state is observable
              in real time. A tool that watches <code>vault.get_recovery_state()</code> alerts you the
              moment a recovery starts, giving you the configured
              window to react.</li>
            <li>For high-value deployments, prefer customers whose <code>parent</code> account is a multisig contract &mdash;
...
{`outlayer vault verify vault.alice.near`}
 ...
 Red flags: vault is <strong>banned</strong> (<code>is_vault_verified</code> returns false even after
 ...
 with <strong>1 NEAR</strong>. With <code>UseGlobalContract</code>
          ...
          <li>
 <strong>Why the vault must keep a NEAR balance (important):</strong>            deriving your per-vault master key calls
 <code>vault.request_master → mpc.request_app_private_key</code> <em>on-chain</em>, signed by the vault&rsquo;s own TEE key — so
 the vault pays for it. NEAR requires the signer to <em>prepay</em>            the full transaction gas up front at a pessimistic gas price
 ...
 gas-prepaying <code>request_master</code>. A vault that was fine
            ...
            balance drifted below the prepayment. So keep a <strong>standing</strong> balance, not a one-shot amount.
          </li>
          <li>
 ...
 registers a new key via <code>propose_tee_key</code>            (permissionless, but the contract&rsquo;s callback
 cross-checks <code>keystore-dao.is_keystore_approved</code>
            before committing — non-approved adds are rolled back).
            Parents can retire stale keys with <code>clear_unused_tee_keys(vec![pubkey, ...])</code>            (parent-only; typo-guarded — panics on unknown keys).
