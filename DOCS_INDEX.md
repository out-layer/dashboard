...
| `/docs/agent-custody` | `dashboard/app/docs/agent-custody/page.tsx` | `.ignore/FIREBLOCKS.md`, `CUSTODY.md`, canonical AI-agent skill at [`out-layer/outlayer-skills/agent-custody/SKILL.md`](https://github.com/out-layer/outlayer-skills/blob/main/agent-custody/SKILL.md) (hosted at https://skills.outlayer.ai/agent-custody/SKILL.md), **confidential-intents agent integration guide** at [`out-layer/coordinator/docs/CONFIDENTIAL_INTENTS.md`](https://github.com/out-layer/coordinator/blob/main/docs/CONFIDENTIAL_INTENTS.md), [`@outlayer/sdk`](https://github.com/out-layer/sdk-js) | **Agent Custody: multi-chain wallet, policy engine, multisig approval (`#verify-approval` — what an approver signs: the vote is bound to `sha256(op_canonical)`, the browser recomputes it before Approve is enabled, the TEE re-evaluates the policy and verifies the approvers at execution, so a request's origin does not matter), gasless transfers, native-NEAR withdrawal (`token=near` unwraps wNEAR via Intents `native_withdraw`), MPC security, NEP-413 message signing, **limit orders** (`/wallet/v1/limit-orders` — a swap rested on 1Click at the owner's price; authorised once and paid out later with no further signature, so gated as an exit: default-DENY `limit_order` capability + transaction type of its own, address rules on `recipient`, amount limits; a thin door onto 1Click's `/v0/orders` — its parameters in, its order out in snake_case with lower-case values, `is_payout_status_final` the only terminal signal; multisig approvers sign the order's terms; a freeze stops new orders but cancels nothing — cancelling is never frozen and the agent's key keeps working for it (`cancel-all`, 50 per call); see `#limit-orders`, source doc `docs/LIMIT_ORDERS.md` in the coordinator repo), confidential intents (private shard `intents.far` — on-chain balances with no public RPC; shield/unshield/withdraw/transfer/swap + privacy disclosure, see `#confidential-intents`), EVM signing (EIP-712 typed data via `/wallet/v1/evm/sign-typed-data` (v4), EIP-191 personal_sign via `/wallet/v1/evm/sign-message`, optional raw-tx via `/wallet/v1/evm/sign-transaction` gated by `evm_sign.raw_tx` — client serializes the unsigned tx, keystore keccak256+signs, NO assembly/nonce/gas/broadcast; one shared secp256k1 `0x` address from `/wallet/v1/address` across ethereum/polygon/base/arbitrum/optimism/bsc/avalanche/hyperevm/hood (`hood` = Robinhood Chain, an Arbitrum L2, also a 1Click deposit/withdraw chain; it carries no USDC, so a deposit intent there names its token); 65-byte `0x` r‖s‖v sigs, v∈{27,28}, low-s, yParity = v-27 for EIP-1559; `evm_sign` policy capability is DEFAULT-DENY under a policy (set `allowed:true`; a no-policy wallet is unrestricted) with `raw_tx` sub-flag DEFAULT-OFF, no `requires_approval` — an EIP-712 sig is itself fund-moving (EIP-3009 ≈ transfer, EIP-2612 ≈ approve), so it grants full authority over the bridged EVM float, never the NEAR-intents balance; client builds/broadcasts), **Solana signing** (off-chain messages via `/wallet/v1/solana/sign-message` — raw-bytes ed25519, SIWS/nacl-verifiable, REJECTS bytes that parse as a valid tx message (the Phantom-style guard, closes the `raw_tx` bypass); transactions via `/wallet/v1/solana/sign-transaction` gated by `solana_sign.raw_tx` — client sends the base64 serialized tx MESSAGE (`tx.serializeMessage()`, ≤1232 bytes), keystore ed25519-signs the bytes as-is, NO assembly/blockhash/fees/broadcast; own base58 ed25519 address from `/wallet/v1/address?chain=solana` (alias `sol` canonicalized); 64-byte base58 sigs; `solana_sign` capability mirrors `evm_sign` — DEFAULT-DENY under a policy, `raw_tx` sub-flag DEFAULT-OFF, no `requires_approval`, fund-moving authority bounded to the Solana float, never the NEAR-intents balance; client builds/broadcasts; source docs `docs/MULTI_CHAIN.md` in near-offshore + coordinator repos), CLI integration (wallet-key login), TypeScript SDK** |
...
**Adding Order**:
1. Determine difficulty level (Beginner → after echo-example, Intermediate → after weather-example, Advanced → at end)
