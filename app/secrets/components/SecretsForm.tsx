...
export function SecretsForm({
  ...
  useEffect(() => {
    ...
    if (prefill.secretName && generationType) {
      // The enclave generates this one, so there is nothing to type. Clear the JSON example:
      // leaving it would suggest an API_KEY is about to be stored alongside, and the guard that
      // ignores it is an exact string comparison — one stray keystroke turns the placeholder into
      // a real secret.
      setSecretsToGenerate([{ id: '1', name: prefill.secretName, generationType }]);
      setPlaintextSecrets('');
    } else if (prefill.secretName) {
      // A named secret with no generation spec is a value the user supplies; give them the key
      // and let them fill in only the value.
      setPlaintextSecrets(`{\n  "${prefill.secretName}": ""\n}`);
    }
  }, [prefill]);
 ...
 className="w-64 px-3 py-2 border border-border-strong rounded-md shadow-sm focus:outline-none focus:ring-accent focus:border-accent"
                  ...
                  <button
                    ...
                    disabled={encrypting}
                    aria-label="Remove this secret row"
                    title="Remove this secret row"
